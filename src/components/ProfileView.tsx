...
import { CheckCircle2, BookOpen, Layers, Award, Trash2, Sparkles, Shield, BrainCircuit, Send, MessageSquareQuote } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-safe-nav max-w-[880px] mx-auto animate-fadeIn">
      {/*
        ...
        Burada bir hesap kartı vardı: giriş/çıkış düğmesi, "Üye" ve
        "Doğrulandı" rozetleri, e-posta, şehir. Anlora'da üyelik yok —
        hepsi kaldırıldı. Kalan şey kullanıcının gerçekten sahip olduğu
        tek kimlik: bu telefondaki ilerlemesi.
      */}
      <div className="parsomen-panel bg-[var(--surface)] rounded-2xl p-6 sm:p-7 border border-[var(--border)] shadow-[0_1px_3px_rgba(30,36,48,0.03)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h2 className="text-xl font-bold text-[var(--text-primary)]">Profilim</h2>
            <p className="text-xs text-[var(--text-secondary)] mt-0.5">
              İlerlemen bu telefonda kaydediliyor.
            </p>
          </div>
        </div>
        ...
        {/*
          ...
          sahne fotoğrafı konmuyor: profil bir tanıtım sayfası değil, ayar
          sayfası. Şeffaf karga 48 dp'de sağda duruyor; dar ekranda satır
          alta kayıyor, hiçbir şeyin üstüne binmiyor.
        */}
      ...
      {/*
        VERİNİN NEREDE DURDUĞU.

        Bu kutu bir zamanlar iki yüzlüydü: sunucu varsa "hesap aç" davetiydi,
        yoksa yedek hatırlatmasıydı. Hesap kalmadığı için tek bir şey söylüyor
        ve o şey her kurulumda doğru.
      */}
      <div className="p-5 rounded-2xl bg-[var(--learning-soft)]/60 border border-[var(--learning-border)] text-[var(--learning-text)] space-y-2">
        <div className="font-bold text-xs flex items-center gap-1.5 text-[var(--learning-text)]">
          ...
          <span>Verilerin bu cihazda saklanıyor</span>
        </div>
        <p className="text-xs text-[var(--learning-text)] leading-relaxed">
          İlerlemen ve setlerin telefonunda tutuluyor; hiçbir yere
          gönderilmiyor. Telefonunu değiştirmeden önce aşağıdan yedek almayı
          unutma.
        </p>
      </div>
