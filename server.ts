...
/*
 * Yapay zekâ mantığı ortak: aynı denetim ve istemler `worker/` altındaki
 * Cloudflare Worker tarafından da kullanılıyor. Tek kaynak olmasının nedeni
 * `shared/ai/validation.ts` başında yazılı.
 */
import type { AiGateway } from './shared/ai/handlers';
import {
...
/**
 * Ortak işleyicilerin beklediği köprünün Node karşılığı.
 *
 * Worker düz `fetch` kullanıyor, burada `@google/genai` istemcisi var. Tek
 * fark bu; istem, doğrulama ve yeniden deneme kuralları ortak dosyada.
 */
function dugumKoprusu(ai: any): AiGateway {
  return {
...
// Kart denetimi `shared/ai/validation.ts` içinde: aynı mantığı Cloudflare
// Worker da kullanıyor, iki kopya tutulmuyor.
...
app.post('/api/ai/generate-word', blockDuringMaintenance, async (req, res) => {
  ...
  try {
    ...
    /*
     * İSTEM VE ÜRETİM DÖNGÜSÜ ARTIK ORTAK.
     *
     * Burada `shared/ai/prompts.ts` ile birebir aynı istemin BİR KOPYASI
     * duruyordu. O dosyanın var oluş gerekçesi tam olarak buydu: "istem
     * metni değişirse iki ortam birden değişsin, biri eski kalmasın."
     * Kopya bırakıldığı için gerçekten de oldu — istemdeki örnek cümle
     * sayısı ikiden üçe çıkarıldığında yalnızca Cloudflare vekili
     * güncellendi, bu sunucu iki cümle üretmeye devam etti.
     *
     * Önbellek ve kota BURADA kalıyor: ikisi de bu ortama özgü. Ortak
     * işleyicinin işi yalnızca istemi kurup modeli çağırmak ve dönen kartı
     * doğrulamak.
     */
...
app.post('/api/ai/validate-senses', async (req, res) => {
  ...
  try {
    ...
    // İstem ortak dosyada; bkz. generate-word'deki açıklama.
    const sonuc = await handleValidateSenses(req.body, dugumKoprusu(ai));
    return res.status(sonuc.status).json(sonuc.body);
  } catch (error: any) {
...
app.post('/api/ai/generate-examples', async (req, res) => {
  ...
  try {
    ...
    // İstem ortak dosyada; bkz. generate-word'deki açıklama.
    const sonuc = await handleGenerateExamples(req.body, dugumKoprusu(ai));
    return res.status(sonuc.status).json(sonuc.body);
  } catch (error: any) {
...
app.get('/api/health', (_req, res) => {
  ...
  return res.json({
    ok: dataWritable,
    /*
     * Hangi özellikler gerçekten karşılanıyor?
     *
     * Arayüz eskiden tek bir soruya bakıyordu: "sunucu var mı?". Bu, sunucu
     * ile Anlora AI'nin hep birlikte geldiğini varsayıyordu. Artık her zaman
     * doğru değil: `worker/` altındaki Cloudflare vekili yalnızca yapay
     * zekâyı karşılıyor, hesap ve bulut yedeğini karşılamıyor. Tek boole ile
     * o kuruluma bağlanan bir uygulama, basıldığında çalışmayan bir giriş
     * düğmesi çizerdi.
     *
     * Tam sunucu hepsini karşılar; yine de açıkça bildirir ki istemci
     * varsaymak zorunda kalmasın.
     */
    capabilities: {
